...
def protected_roundtrip(source: Path) -> dict:
    ...
    values = make_dialogue_values(
        doc.events_format, start_ms=1000, end_ms=2000, text="TEST", style="SF-ZH"
    )
    text = render_from_template(
        doc, [(1000, 1_000_000, build_event_line(doc.events_format, values))]
    )
    retained = sum(1 for event in protected if event.raw_line in text)
    return {
        "protected": len(protected),
        "retained": retained,
        "roundtrip": retained == len(protected),
    }
...
def main() -> int:
    ...
    parser.add_argument(
        "--output", type=Path, default=Path("verification/verification-results.json")
    )
